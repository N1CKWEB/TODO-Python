...
class ClienteDAO():
   ...
   _ACTUALIZAR="UPADTE Cliente SET nombre=%s , apellido=%s,  membresia = %s WHERE id_cliente=%s"
   ...
   def eliminar_bd(cls,cliente):
       with CursorDelPool() as cursor:
         cursor.execute(cls._ELIMINAR, (cliente._id_persona))
         log.debug(f"Persona eliminada con id: {cliente._id_cliente}")  
         return cursor.rowcount
